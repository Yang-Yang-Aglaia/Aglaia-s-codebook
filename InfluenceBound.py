#%%
__all__ = ["influence", "DriftConstant", "DriftLinear", "InfluenceBound"]
...
class Influence(Dependence):
    ...
    def get_matrix(self, x, t, dx, dt, conditions, **kwargs):
        ...
        """
        # 计算方法
        influence = self.get_influence(x=x, t=t, dx=dx, dt=dt, conditions=conditions, **kwargs)
        if np.isscalar(influence):  # 是否是标量，返回值为逻辑值
            return TriDiagMatrix(up=0.5*dt/dx * influence * np.ones(len(x)-1),
                                 down=-0.5*dt/dx * influence * np.ones(len(x)-1))
        else:
            return TriDiagMatrix(up=0.5*dt/dx * influence[1:],
                                 down=-0.5*dt/dx * influence[:-1])
    ...
    def get_flux(self, x_bound, t, dx, dt, conditions, **kwargs):
        ...
        """
        # np.sign为取符号函数
        return 0.5*dt/dx * np.sign(x_bound) * self.get_influence(x=x_bound, t=t, dx=dx, dt=dt, conditions=conditions, **kwargs)

    def get_influence(self, t, x, conditions, **kwargs):
        ...
        """
        raise NotImplementedError("Drift model %s invalid: must define the get_influence function" % self.__class__.__name__)


@paranoidclass
class InfluenceConstant(Influence):
    ...
    """
    name = "constant"
    required_parameters = ["influence"]
    @staticmethod
    def _test(v):
        assert v.dinfluence in Number()
    @staticmethod
    def _generate():
        yield InfluenceConstant(drift=0)
        yield InfluenceConstant(drift=1)
        yield InfluenceConstant(drift=-1)
        yield InfluenceConstant(drift=100)
    @accepts(Self)
    @returns(Number)
    def get_influence(self, **kwargs):
        return self.influence


@paranoidclass
class InfluenceLinear(Influence):
    ...
    """
    name = "linear_xt"
    required_parameters = ["influence", "x", "t"]
    @staticmethod
    def _test(v):
        assert v.influence in Number()
        assert v.x in Number()
        assert v.t in Number()
    @staticmethod
    def _generate():
        yield InfluenceLinear(influence=0, x=0, t=0)
        yield InfluenceLinear(influence=1, x=-1, t=1)
        yield InfluenceLinear(influence=10, x=10, t=10)
        yield InfluenceLinear(influence=1, x=-10, t=-.5)
    ...
    def get_influence(self, x, t, **kwargs):
        return self.influence + self.x*x + self.t*t


@paranoidclass
class InfluenceBound(Influence):
    """Drift dependence: drift rate varies linearly with position and time.
    ...
    def _test(v):
        ...
        assert v.t in Number()
    @staticmethod
    def _generate():  # 调用时写函数中定义的格式
        yield InfluenceBound(influence=0, x=0, t=0)
        yield InfluenceBound(influence=1, x=-1, t=1)
        yield InfluenceBound(influence=10, x=10, t=10)
        yield InfluenceBound(influence=1, x=-10, t=-.5)
    # We allow this function to accept a vector or a scalar for x,
    # because if we use list comprehensions instead of native numpy
    # multiplication in the get_matrix function it slows things down by
    # around 100x.
    ...
    @returns(Or(Number, NDArray(d=1, t=Number)))
    @ensures("np.isscalar(x) <--> np.isscalar(return)")
    def get_influence(self, G, q, x, t, **kwargs):  # G表示他人选择，q表示改变量或改变程度
        return self.influence + self.G*q + self.x*x + self.t*t