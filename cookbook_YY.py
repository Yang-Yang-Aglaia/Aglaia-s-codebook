...
class DriftRT(Drift):
    name = "Drift depends linearly on RT"
    required_parameters = ["driftrt"] # <-- Parameters we want to include in the model
    required_conditions = ["rt"] # <-- Task parameters ("conditions"). Should be the same name as in the sample.
...
from ddm import Drift
class DriftRTRewBias(Drift):
    name = "Drift depends linearly on RT, with a reward bias"
    required_parameters = ["driftrt", "rewbias"] # <-- Parameters we want to include in the model
    required_conditions = ["rt", "highreward"] # <-- Task parameters ("conditions"). Should be the same name as in the sample.
...
from ddm import Drift
class DriftRTLeak(Drift):
    name = "Leaky drift depends linearly on RT"
    required_parameters = ["driftrt", "leak"] # <-- Parameters we want to include in the model
    required_conditions = ["rt"] # <-- Task parameters ("conditions"). Should be the same name as in the sample.

    # We must always define the get_drift function, which is used to compute the instantaneous value of drift.
    def get_drift(self, x, conditions, **kwargs):
        return self.driftcoh * conditions['rt'] + self.leak * x
