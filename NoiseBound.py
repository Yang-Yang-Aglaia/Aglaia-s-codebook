#%%
__all__ = ["Noise", "NoiseConstant", "NoiseLinear", "NoiseBound"]
...
class NoiseBound(Noise):
    ...
    @staticmethod
    def _generate():
        yield NoiseLinear(noise=math.sqrt(2*D), x=0, t=0)  # 运用时写这个表达式就可以了
        yield NoiseLinear(noise=math.sqrt(2*D), x=-1, t=1)
        yield NoiseLinear(noise=math.sqrt(2*D), x=10, t=10)
        yield NoiseLinear(noise=math.sqrt(2*D), x=-10, t=-.5)
    @accepts(Self, Or(Number, NDArray(d=1, t=Number)), Positive0)
    @returns(Or(Positive, NDArray(d=1, t=Positive)))
    @requires('self.noise + self.x*x + self.t*t > 0') # Noise can't go below zero
    @ensures("np.isscalar(x) <--> np.isscalar(return)")
    def get_noise(self, G, q, x, t, **kwargs):  # 噪音计算表达式
        return (self.noise + self.x*x + self.t*t) + G*q  # 直接将他人选择以及改变量加在后面